    ...
    if (client_vi.register_commands(True, strings)):
        ...
        while (enable):
            ...
            for event in events:
                ...
                if event.index == 1: # acquire
                    # Save last x frames as acquisition for target point cloud
                    data_depth_option += data_depth[-acquire_frames:]
                    data_grey_option += data_grey[-acquire_frames:]
                
                if event.index == 2: # clear
                    # Clear saved acquisitions
                    data_depth_option = []
                    data_grey_option = []

                if event.index == 3: # register
                    # Run registration based on user saved acquisitions
                    try:
                        save_folder = Path.cwd() / 'data' / 'demo_registrations' / f'registration_{model}_{acq_name}_{acq_nr}'

                        if not save_folder.is_dir(): save_folder.mkdir()
                    
                        target_pcd = volume_integration(data_depth_option, data_grey_option, lt=lt, save_folder=save_folder)
                        register(host, model, target_pcd, data_si[-eyes_to_send:], save_folder=save_folder)
                ...
                if event.index == 4: # cadaver
                    model = 4
                if event.index == 5: # skull
                    model = 1
                if event.index == 6: # chest
                    model = 2
                if event.index == 7: # spine
                    model = 3
                if event.index == 8: # abdomen
                    model = 5

...
class ipc_umq(_context_manager):
    ...
    def pull(self, buffer, is_transform=False):
        return self.pull_n(buffer.get_count(), is_transform)
    
    def pull_n(self, count, is_transform):
        if is_transform:
            return np.frombuffer(self._client.download(4 * 16, ChunkSize.SINGLE_TRANSFER), dtype=np.float32)
        else:

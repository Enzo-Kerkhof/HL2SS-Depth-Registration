...
static std::queue<MQ_MSG> g_queue_sot;
static bool send_pose_msg = false;
...
static DWORD WINAPI MQ_EntryPoint_Send(void *param)
{
	...
	if (status != sizeof(id) && status != msg.size) { break; }
	}
	while (WaitForSingleObject(g_event_error, 0) == WAIT_TIMEOUT);
...
// OK
UNITY_EXPORT
void MQ_SO_Push(uint32_t id, float* model_transform = nullptr)
{
	CriticalSection cs(&g_lock_so);
	if (model_transform != nullptr)
	{
		MQ_MSG msg;
		msg.size = 64U;	// 16xsizeof(float)
		msg.data = malloc(msg.size);
		if (msg.data != NULL) {
			memcpy(msg.data, model_transform, msg.size);
		}
		g_queue_sot.push(msg);
		send_pose_msg = true;
	}
	else
	{
